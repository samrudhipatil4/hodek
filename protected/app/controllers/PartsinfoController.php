...
class PartsinfoController extends BaseController {
	...
	public $module = 'partsinfo';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Partsinfo();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','partsinfo.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('parts_info'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','partsinfo.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','partsinfo.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('parts_info');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'partsinfo/add/'.$id.'?md='.$md.$trackUri :  'partsinfo?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('partsinfo/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDestroy()
	{
		...
		return Redirect::to('partsinfo?md='.Input::get('md'));
	}			
