   ...
   public function get_final_approval($id){
    ...
    public function get_document_verification_status($id){
    	$data1=DB::table('request_progress_status_temp')
    		  ...
    		  ->where('status',16)
    		  ->get();
    	$data2=DB::table('activity_completion_sheet_verify')
    		  ->select('status')
    		  ->where('request_id',$id)
    		  ->get();
    	// print_r($data1);exit();
    	if(empty($data1)){//echo 'in green';exit;

			$status=1;
			$class='-';
			$text='-';

		}else if(!empty($data1) && empty($data2)){//echo 'in yellow';exit;
	...
	function get_user_by_role($group_id){
	 ...
	 function get_team_leader_name($request_id){
		...
		$users  = DB::table('tb_users')
					...
					->select('tb_users.id','tb_users.first_name','tb_users.last_name','tb_users.email','add_update_initial_sheet.stock')
