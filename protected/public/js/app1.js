...
$http
        ...
        $scope.getInitDept = function(r_id){
        $http
        .get(appurl + 'changes/get_inititor_dept/'+r_id)
        .success(function(data, status, headers, config) {
    ...
    $scope.addRequest_fdraft = function(requestForm,type,request_id,id) {
       ...
       var file1=$("#changeReqFile").val();
        $scope.submitted= true;
        if (requestForm.$invalid) {
          
           $scope.invalidSubmitAttempt = true;
           if(file =='' &&  file1== undefined){
          
             document.getElementById("demo").innerHTML = 'This field is required.';
            
        }
           
        }else if(file =='' &&  file1== undefined){
          
             document.getElementById("demo").innerHTML = 'This field is required.';
             $scope.invalidSubmitAttempt = true;
        } else {
           var hod = $("#Authority").val();
           if(hod != ""){
            ...
            if (project_code == '? string:1 ?' ||project_code == '' ||project_code == '? string:2 ?') {
            ...
            $http
                ...
                .success(function(data, status, headers, config) {
                    ...
                    if (value[1] == 1) {
                        ...
                        location.href = appurl + 'changes/edit_change_request/'+request_id;
                    } else if (data == 0) {
                        $("#fade").show();
                         $scope.isDisabled = true;
    ...
    $scope.deleteAttachement = function(id,name){
        
         $http
                    .post(appurl + 'deleteChangeReqAttachment/' +id + '/'+name)
                    .success(function (data, status, headers, config) {
                        
                        if( data.indexOf('Success') != -1 )
                        {
                            $("#tr_"+id).hide('slow').  remove();
    ...
    $scope.EditRequest = function(requestForm,id,page) {
        
        
        flag = 0;
        if( $("#statemultiselect11").val() == null  )
        {   
            $("#statemultiselect11_span").removeClass('ng-hide')
            flag = 1;
            //$("#statemultiselect11_span").show();
            //return false;
        }
        
        var changeType = $("#changeType").val();
        changeType = changeType.split('@');
        if( changeType[1] == 'Single' )
        {
            if( $("#design").val() == null || $("#design").val() == "" )
            {
                
                $("#customersingle").removeClass('ng-hide')
                flag = 1;
                //$("#statemultiselect11_span").show();
                //return false;
            }
        ...
        {
            if( $("#statemultiselect").val() == null  )
            {   
            
                $("#customerMulti").removeClass('ng-hide')
                flag = 1;
       ...
       if (requestForm.$invalid) {
           ...
           if(file =='' && file1== undefined){
            ...
            $scope.request.multi_purpose=$scope.request.selected_items_for_purpose;
            $scope.request.multi_subtype=$scope.request.selected_items_for_multi_subtype;

            $scope.request.Approval_Authority=$scope.request.Approval_Authority;
            $scope.request.uploadFileName = $("#uploadFileName").val();
            ...
            $scope.request.dt1=$('#calender').val();
            
            if($scope.request.dt1 == undefined){
               
                 $scope.request.dt=$('#calender1').val();
            }else{
            ...
            if(page == "modify"){
                 $http
                .post(appurl + 'changes/updaterequest1/'+id, data)
                .success(function(data, status, headers, config) {
                      var data1 = data.split('@');
                     if(data1[0]=='1'){
                        $.simplyToast('file'+data1[1]+'not synchronized to server.Please upload again', 'warning');
    ...
    //For Dynamic Change Type
    $scope.fill_customer_name = function(id,url){
    ...
    $scope.removeAllPart = function(id,url){
    ...
    $http
        ...
        $http
                    ...
                    .success(function(data, status, headers, config) {
                         ...
                         //alert(JSON.stringify($scope.stakeholder));
...
//====================================================================
    ...
    $scope.removeAll = function (id) {
        ...
        $http
            .post(appurl + 'changes/delete_all_parts_change_request/'+id)
            .success(function(data, status, headers, config) {
                 //$scope.request=data[0];


                $scope.request_part = [];
                $scope.request_part = [{ extNumber: "", extName: ""}];
    ...
    $scope.editrequestinfo=function(id){ //alert("first");
        ...
        $http
            .get(appurl + 'changes/getCustomerCommDecision/'+id)
            .success(function(data, status, headers, config) {
            ...
            .success(function(data, status, headers, config) {
              // alert(data);
                ...
                var date = data.created_date;
                    ...
                    var yr = year.split("-")[0];
                    var mon = year.split("-")[1];
                    var dd = year.split("-")[2];

                    $scope.request.created_date1 = dd + "/" + mon + "/" + yr;
                var type_id= data.changeType.split('@');
                 $scope.fillSubChangeType(type_id[0]);
                $scope.member='';
                $scope.members='';
               if(data.code != ""){
                $scope.drpdwn =true;
                ...
                $scope.getStage(data.code);
                $scope.checkPrjApply(data.business);


                $scope.changeReqFile=data.changeReqFile;
                 $scope.cntOfFile=data.changeReqFile.length;
             
                $scope.count = data.cntOfChangeReq;
               ...
               // alert($scope.selectedStates);
                ...
                $scope.request.selected_items_for_purpose=data.change_purpose;

                // alert(data.change_purpose);
