...
class ProjectManagerController extends BaseController {
	...
	{ 
		...
		$this->layout->nest('content','ProjectManager.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('tb_project_manager'); 
		}
		...
		$this->layout->nest('content','ProjectManager.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','ProjectManager.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$data = $this->validatePost('tb_project_manager');
			
		$allTblData=$this->getCustCommData();
			$flag = 0;
			foreach ($allTblData as $row) {
				
				if($row['plant_code']  == $data['plant_code'] && $row['change_stage']  == $data['change_stage'] && $row['stakeholder']  == $data['stakeholder'] && $row['proj_mgr_id']  == $data['proj_mgr_id'] ){
					$flag = 1;
				}
			}
			if($flag == 1){
				...
				return Redirect::to('ProjectManager');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'ProjectManager/add/'.$id.'?md='.$md.$trackUri :  'ProjectManager?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('ProjectManager/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function getCustCommData(){
		$data= DB::table('tb_project_manager')->get();
		$allData =[];
		if(!empty($data)){
			foreach ($data as $row) {
				$allData[]  = array(
