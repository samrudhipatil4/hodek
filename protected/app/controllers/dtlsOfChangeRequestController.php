...
class dtlsOfChangeRequestController extends BaseController
{
            ...
            if(!strcmp((String)$input->startdate,(String)$input->enddate)){
             ...
             $query =DB::table('changerequests')
                   ...
                   ->leftJoin('request_progress_status','request_progress_status.request_id','=','changerequests.request_id')
                    ...
                    if($input->stakeholder!= '? undefined:undefined ?'){
                        $query->where('changerequests.stakeholder',$input->stakeholder);
                         $stakeholder=$input->stakeholder;
                    }else{
                        $stakeholder='';
                        }
                        if($input->purpose!= '? undefined:undefined ?'){
                        $query->where('changerequests_purposechange.purpose_id',$input->purpose);
          ...
          //    print_r($users);exit();
             foreach ($users as $user){
                ...
                $data[] = array(
                    ...
                    'initiation_dt'=>$this->get_custom_date1($user->dt),
                    ...
                    'impdate'=>$this->get_custom_date1($user->currentTime),
                    'actualImpDate' =>$this->actualImpDate($user->request_id),
    ...
    public function actualImpDate($rid){
       
            $data=DB::table('befor_after_status_option_attachment')
                    ->select('post_date')
                    ->where('request_id',$rid)
                    ->get();
                    if(!empty($data)){
                        return $data[0]->post_date;
                    }
    ...
    public function getstatus($status,$r_id){
        ...
        if($status == 5){
            $msg="Change Request is drafted";
              ...
              $permanentClose=DB::table('request_progress_status')
                    ->select('message')
                  ...
                  if(!empty($holdId)){
                     $msg="Change Request is holded";
                  }else if(!empty($permanentClose)){
                    $msg=$permanentClose[0]->message;
                  }
                  else{
                    ...
                    // ->where('request_progress_status.close',0);
                     // ->get();
                      ...
                      if(isset($input->project)&& !empty($input->project)){
                        ...
                        if(isset($input->stakeholder)&& !empty($input->stakeholder)){
                        $query->where('changerequests.stakeholder', $input->stakeholder);
                        $stakeholder=$input->stakeholder;
                        }else{
                        $stakeholder='';
    ...
    public function getPurpose(){
        $data=DB::table('changerequest_purpose')
                ->select('*')
                ->where('status','active')
                ->get();
                return $data;
