...
class materialMasterController extends BaseController {
	...
	public $module = 'materialMaster';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new materialMaster();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','materialMaster.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('apqp_material_master'); 
		}
		...
		$this->layout->nest('content','materialMaster.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('apqp_material_master');
			try{
			...
			$redirect = (!is_null(Input::get('apply')) ? 'materialMaster/add/'.$id.'?md='.$md.$trackUri :  'materialMaster/add/'.$id.'?md='.$md.$trackUri);
			      
			       Session::flash('messagetext', 'Duplicate material code');	
			...
			// Redirect after save	
			$md = str_replace(" ","+",Input::get('md'));
			$redirect = (!is_null(Input::get('apply')) ? 'materialMaster/add/'.$id.'?md='.$md.$trackUri :  'materialMaster?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('materialMaster/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDestroy()
	{
		...
		if(count(Input::get('id')) >=1)
		{
			...
			$checkInProject=DB::table('apqp_project_material')
							->select('id')
							->where('material_id',$id[0])
							->get();			
				if(!empty($checkInProject)){
					Session::flash('messagetext', 'You can not delete material.');	
			Session::flash('msgstatus','error');
				}else{
