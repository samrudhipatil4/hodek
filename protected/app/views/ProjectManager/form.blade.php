  ...
  <div class="page-content row">
    ...
    <div class="page-header">
      ...
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('ProjectManager?md='.$filtermd) }}">{{ $pageTitle }}</a></li>
...
<div class="sbox animated fadeInRight">
	...
	<div class="sbox-content"> 	
		 ...
		 {{ Form::open(array('url'=>'ProjectManager/save/'.SiteHelpers::encryptID($row['id']).'?md='.$filtermd.$trackUri, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
<div class="col-md-12">
						<fieldset><legend></legend>
							...
							class='select2 '  required  ></select> 
								  ...
								  <div class="form-group  " >
									...
									<div class="col-md-6">
									   <select name='proj_mgr_id' rows='5' id='proj_mgr_id'  
							class='select2 '  required  >
							<option value="">--Please Select--</option>
							<?php foreach ($member as $row1) { ?>
								<option value="<?php echo $row1['id'];?>" <?php if($row['proj_mgr_id']== $row1['id']){ echo "selected='selected'";}?>><?php echo $row1['name'];?></option>
							<?php } ?>
							</select> 
									 </div> 
									 <div class="col-md-2">
			...
			<div style="clear:both"></div>	
				  ...
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('ProjectManager?md='.$masterdetail["filtermd"].$trackUri) }}' " class="btn btn-success btn-sm "><i class="fa  fa-arrow-circle-left "></i>  {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#plant_code").jCombo("{{ URL::to('ProjectManager/comboselect?filter=plant_code:plant_id:plant_code') }}",
		{  selected_value : '{{ $row["plant_code"] }}' });
		 
		 $("#change_stage").jCombo("{{ URL::to('ProjectManager/comboselect?filter=tb_change_stage:change_stage_id:stage_name') }}",
		{  selected_value : '{{ $row["change_stage"] }}' });

		 $("#stakeholder").jCombo("{{ URL::to('ProjectManager/comboselect?filter=tb_stakeholder:id:name') }}",
