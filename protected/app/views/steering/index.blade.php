...
<div class="page-content row">
	...
	<div class="page-content-wrapper m-t">
		<ul class="nav nav-tabs" style="margin-bottom:10px;">
			<li class="active"><a href="{{ URL::to('steering')}}"> Steering Users </a></li>
		...
		<div class="sbox animated fadeInRight">
			...
			<div class="sbox-content">
				<div class="toolbar-line">
							...
							@if($access['is_add'] ==1)
			   		<a href="{{ URL::to('steering/add?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
					<i class="fa fa-plus-circle text-info"></i>&nbsp;{{ Lang::get('core.btn_create') }}</a>
					@endif  
					@if($access['is_excel'] ==1)
						<a href="{{ URL::to('steering/download') }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
							<i class="fa fa-download text-warning"></i>&nbsp;{{ Lang::get('core.btn_download') }} </a>
					@endif
				</div>

				{{ Form::open(array('url'=>'steering/destroy/', 'class'=>'form-horizontal' ,'ID' =>'SximoTable' )) }}
						...
						@foreach ($rowData as $row)
							<tr>
								...
								<td>
									...
									@if($access['is_edit'] ==1)
										<a  href="{{ URL::to('steering/add/'.$id)}}"  class="tips btn btn-xs btn-white"  title="{{ Lang::get('core.btn_edit') }}"> <i class="fa fa-edit"></i></a>
									@endif
		...
		<script>
			$(document).ready(function(){
				...
				$('.do-quick-search').click(function(){
					$('#SximoTable').attr('action','{{ URL::to("steering/multisearch")}}');
					$('#SximoTable').submit();
				});
