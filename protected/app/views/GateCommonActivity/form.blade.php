  ...
  <div class="page-content row">
    ...
    <div class="page-header">
      ...
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('GateCommonActivity?md='.$filtermd) }}">{{ $pageTitle }}</a></li>
...
<div class="sbox animated fadeInRight">
	...
	<div class="sbox-content"> 	
		 ...
		 {{ Form::open(array('url'=>'GateCommonActivity/save/'.SiteHelpers::encryptID($row['id']).'?md='.$filtermd.$trackUri, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
<div class="col-md-12">
						<fieldset><legend> Common Activity Master</legend>
						 ...
						 @else 
							...
							class='select2 '  required  ></select> 
									...
									<div class="col-md-6">
									 {{ Form::hidden('activity_type', 'C',array('class'=>'form-control', 'placeholder'=>''  )) }}
									 </div> 
									 <div class="col-md-2">
			...
			<div style="clear:both"></div>	
				  ...
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('GateCommonActivity?md='.$masterdetail["filtermd"].$trackUri) }}' " class="btn btn-success btn-sm "><i class="fa  fa-arrow-circle-left "></i>  {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
     ...
     var comm=$("#commodity_name").val();
	    	...
	    	if(gate != "" && type != '' && comm != ''){
	    		$.ajax({
                   url:base_url+'getActivity',
                    type: 'POST',
                    data:{gate:gate,activity_type:type,commodity:comm},
                    success: function (data) {
                    	$('#previous_reference_activity').html(data);
                  }
                });
