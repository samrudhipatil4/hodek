<?php
class HorizDeploymentController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'HorizontalDeployment';
	static $per_page	= '10';
	...
	public function __construct() {
		...
		$this->model = new HorizontalDeployment();
		...
		$this->data = array(
			...
			'pageModule'=> 'HorizontalDeployment',
	...
	{ 
		...
		$this->layout->nest('content','HorizontalDeployment.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('tb_dynamicHorizDeploy'); 
		}
		...
		$this->layout->nest('content','HorizontalDeployment.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','HorizontalDeployment.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$data = $this->validatePost('tb_dynamicHorizDeploy');
			...
			foreach ($allTblData as $row) {
				...
				if($row['plant_id']  == $data['plant_id']  && $row['stakeholder']  == $data['stakeholder'] && $row['member']  == $data['member'] && $row['change_stage'] == $data['change_stage'] ){
					$flag = 1;
				}
			}
			if($flag == 1){
				...
				return Redirect::to('HorizontalDeployment');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'HorizontalDeployment/add/'.$id.'?md='.$md.$trackUri :  'HorizontalDeployment?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('HorizontalDeployment/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function getData(){
		$data= DB::table('tb_dynamicHorizDeploy')->get();
		$allData =[];
		if(!empty($data)){
			foreach ($data as $row) {
				$allData[]  = array(
