...
class BaseModel extends Eloquent  {
	...
	public function getSteeringComm(){
	...
	public function coogetUser(){
		$member = DB::table('tb_users')
				->select('id','first_name','last_name')
				// ->where('tb_users.group_id','LIKE','%15%') 
				->get();
				$data=[];
				foreach ($member as $row) {
					$data[] =  array(
	...
	public function cftgetUser(){
		$member = DB::table('tb_users')
				...
				foreach ($member as $row) {
					$data[] =  array(
						'id' => $row->id,
						'name'	=> $row->username.' - '.$row->first_name.' '.$row->last_name
					 );
				}
