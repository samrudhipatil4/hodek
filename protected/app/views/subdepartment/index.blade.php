...
<div class="sbox animated fadeInRight">
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('subdepartment/add?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
			...
			<a href="{{ URL::to('subdepartment/download?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
	...
	{{ $details }}
	 ...
	 {{ Form::open(array('url'=>'subdepartment/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) }}
    ...
    <table class="table table-striped ">
        ...
        <tbody>
            ...
            @foreach ($rowData as $row)
                <tr>
					<td width="30"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="{{ $row->sub_dep_id }}" />  </td>									
				 ...
				 <td>
						...
						{{--*/ $id = SiteHelpers::encryptID($row->sub_dep_id) /*--}}
					 	
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('subdepartment/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit"></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("subdepartment/multisearch")}}');
		$('#SximoTable').submit();
	});
