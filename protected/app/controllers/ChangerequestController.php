           ...
           // print_r($input);exit();
            ...
            if(!empty($input->uploadFileName)){
              ...
              foreach($fileName as $file){
                ...
                $filepath = Config::get('app.site_root').'uploads/changeRequest'.'/'.$file;
            ...
            if ($input->type == 1) {
                ...
                $assignedto=$initiator_name['name'];
                 $query =DB::table('changerequests')
                    ->select('changerequests.changeType','changerequests.created_date','changerequests.request_id','changerequests.Purpose_Modification_Details')
                ...
                $query1=DB::table('tbl_change_type')
                  ...
                  $Mailparts = DB::table('tbl_parts_info')
                          ->select('tbl_parts_info.part_number','tbl_parts_info.part_name')
                          ->where('tbl_parts_info.request_ids', '=', $request_id)
                          ->get(); 
                  $MailCustomer = DB::table('changerequests_customer')                            
                                  ->leftJoin('customer','customer.CustomerId','=','changerequests_customer.customer_id')
                ...
                $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'task_assigned_to'=>$assignedto,'url'=>$url,'request_id'=>$cmNo,
                               'Close_status'=>$close_status,'Parts'=>$Mailparts,'Customer'=>$MailCustomer,'mod_details'=>$query[0]->Purpose_Modification_Details);
      ...
      $checkClose = DB::table('request_progress_status')
        ...
        if ($input->radioStatus == 3) {
           ...
           $query =DB::table('changerequests')
            ...
            $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'task_assigned_to'=>$assignedto,
              'url'=>$url,'request_id'=>$cmNo,'Close_status'=>$close_status,'comment'=>$comment,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
...
// print_r($init_id);exit();
            ...
            $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'task_assigned_to'=>$assignedto,'url'=>$url,
              'request_id'=>$cmNo,'Close_status'=>$close_status,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
    ...
    {//exit;
        if (Request::isJson()) {
            ...
            $input = (object)Input::all();
              ...
              $fileName =explode(",", $input->uploadFileName);
            if(!empty($input->uploadFileName) || $input->uploadFileName != ""){
              $filelist ="";
              $f=0;
              foreach($fileName as $file){
                 // print_r($file);
        ...
        if (Request::isJson()) {
          ...
          //----start code for tracking Sheet----
            ...
            foreach ($data_11 as $value) {
                ...
                $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'task_assigned_to'=>$assignedto,'url'=>$url,
                  'request_id'=>$cmNo,'Close_status'=>$close_status,'Comment'=>$comment,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
     ...
     $checkClose = DB::table('request_progress_status')
      ...
      if($StageType[0]->change_stage == 1){
        ...
        $data3 = DB::table('request_progress_status')
            ...
            $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'url'=>$url,
              'request_id'=>$cmNo,'Close_status'=>$close_status,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
            if($this->check_netconnection())
            {
             ...
             foreach ($data1 as $value) {
                ...
                $query1=DB::table('tbl_change_type')
                  ...
                  $MailCustomer = DB::table('changerequests_customer')                            
                        ...
                        $data_1 = array('firstname' => $contactName, 'description' => $message, 'assigned_task_by' => $assignedby, 'url' => $url, 'request_id' => $cmNo,
                         'Close_status' => $close_status,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
    ...
    public function add_pending_approval($id, $id1)
    {
        ...
        //---------------------------------
            ...
            if ($data2[0]->total > 1) { //echo "in 
                ...
                $data_1 = array('firstname' => $contactName, 'description' => $message, 'assigned_task_by' => $assignedby, 'url' => $url,
                 'request_id' => $cmNo, 'Close_status' => $close_status, 'comment' => $comment,
'Customer'=>$MailCustomer,'Parts'=>$Mailparts
,'mod_details'=>$query[0]->Purpose_Modification_Details);
             ...
             $stage=DB::table('changerequests')
                ...
                $query1=DB::table('tbl_change_type')
                  ...
                  $cmNo=$this->generate_cm_no_search($query1[0]->change_type_name,$query[0]->created_date,$query[0]->request_id);
                        ...
                        $data_1 = array('firstname' => $contactName, 'description' => $message, 'assigned_task_by' => $assignedby, 'url' => $url,
                         'request_id' => $cmNo, 'Close_status' => $close_status,'Customer'=>$MailCustomer,'Parts'=>$Mailparts);
  ...
  // End of CFT team Representative User
    ...
    {
      $checkAdminModify = Request::segment(1);

        if (!empty(Input::get('delete_attachment'))) {

            if (!empty(Input::get('attachment_name'))) {
               if($checkAdminModify != "changeRequestModifyDetails"){
                           $checkClose = DB::table('request_progress_status')
                          ->select('close')
                          ->where('request_id',Request::segment(3))
                          ->where('id',Request::segment(4))
   ...
   public function submit_customer_communication_list($id1,$id)
    {
      $checkClose = DB::table('request_progress_status')
        ...
        if ($count >= 1) {
             $filedata= DB::table('customer_communication_list_attachments')
                    ->select('doc_name')
                    ->where('request_id',$input->request_id)
                    ->get();
                    $filelist ="";
              ...
              foreach($filedata as $file){
                $filepath = Config::get('app.site_root').'uploads/attachments'.'/'.$file->doc_name;
                if(!file_exists($filepath)){
                  $f=1;
                  $filelist=$filelist.' '.$file->doc_name.",";
              }
              }
             
              if($f == 1){
                return Redirect::back()->with('message', SiteHelpers::alert('error', ' file'.$filelist.' not synchronized to server.Please remove it and upload again.'));
              }
  ...
  return $total[0]->total;
               ...
               //------------end------------   
                ...
                $query1=DB::table('tbl_change_type')
                  ...
                  $MailCustomer = DB::table('changerequests_customer')                            
                    ...
                    $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'url'=>$url,'request_id'=>$cmNo,
                            'Close_status'=>$close_status,'comment'=>$comment,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
    ...
    public function customer_verify($id1)
    {
        ...
        if ($input->radioStatus == 1) {
          ...
          $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'url'=>$url,'request_id'=>$cmNo,
            'Close_status'=>$close_status,'comment'=>$comment,
'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
            ...
            if ($data1[0]->total > 0) {
             ...
             $comment = $input->comment;
              ...
              $comment='';
                ...
                $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'url'=>$url,'request_id'=>$cmNo,
                  'Close_status'=>$close_status,'Customer'=>$MailCustomer,'Parts'=>$Mailparts
,'mod_details'=>$query[0]->Purpose_Modification_Details);
      ...
      $destinationPath = 'uploads/PTRDocument'; // upload path
        ...
        if (Input::hasFile('doc')) {
            ...
            foreach ($files as $file) {
                ...
                $file = Config::get('app.site_root').'uploads/PTRDocument'.'/'.$filename1;

                if(!file_exists($file)){
                   return Redirect::back()->with('message', SiteHelpers::alert('error', ' file'.$file->attachment_file.' not synchronized to server.Please upload again'));
                }
...
,'mod_details'=>$query[0]->Purpose_Modification_Details);
        ...
        foreach ($involveusers as $key => $value) {
           $usersdtls = DB::table('tb_users')
             ...
             $data_1 = array('firstname'=>$usersdtls[0]->full_name,'description'=>$message,
              'CMNo'=>$CMNo,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
          ...
          //  print_r($files);exit;
            ...
            foreach ($files as $file) {
               ...
               if (!in_array(strtolower($ext), $a)) {
                ...
                $filepath = Config::get('app.site_root').'uploads/before_after_status_option_attachment/'.preg_replace("/[^A-Za-z0-9\_\.]/", '', $filename);
                if(!file_exists($filepath)){
                  return Redirect::back()->with('message', SiteHelpers::alert('error', 'file '.$filename.' not synchronized to server.Please upload again.'));
                }
...
,'mod_details'=>$query[0]->Purpose_Modification_Details);
          ...
          DB::table('tb_final_reject')->insert(
           ...
           $users = DB::table('tb_users')
               ...
               // $close_status = $last_id_req;
                ...
                $data_1 = array('firstname' => $contactName, 'description' => $message, 'assigned_task_by' => $assignedby, 'url' => $url, 'request_id' => $cmNo,
                  'comment'=>$input->rejectcoment,'Customer'=>$MailCustomer,'Parts'=>$Mailparts,'mod_details'=>$query[0]->Purpose_Modification_Details);
  ...
  // ******** Mail to all members involved in change request *************
            $CM_dtls=DB::table('changerequests')
                    ->leftJoin('tbl_change_type', 'changerequests.changeType', '=', 'tbl_change_type.change_type_id')
                    ->select('change_type_name','created_date','changerequests.status','change_stage','changerequests.Purpose_Modification_Details')
                    ->where('request_id',$id)
                    ->get();
        ...
        foreach ($involveusers as $key => $value) {
           $usersdtls = DB::table('tb_users')
                ...
                $Mailparts = DB::table('tbl_parts_info')
                          ->select('tbl_parts_info.part_number','tbl_parts_info.part_name')
                          ->where('tbl_parts_info.request_ids', '=', $id)
                          ->get(); 
                  $MailCustomer = DB::table('changerequests_customer')                            
                                  ->leftJoin('customer','customer.CustomerId','=','changerequests_customer.customer_id')
                                  ->select('customer.FirstName')
             ...
             $data_1 = array('firstname'=>$usersdtls[0]->full_name,'description'=>$message,
              'CMNo'=>$CMNo,'Customer'=>$MailCustomer,'Parts'=>$Mailparts
,'mod_details'=>$CM_dtls[0]->Purpose_Modification_Details);
             $loginuser=Session::get('uid');
             // print_r($usersdtls[0]->id);exit();
    ...
    public function assign_task_to_next_step_frm_activity_monitoring($id,$id1){
      $checkClose = DB::table('request_progress_status')
        ...
        if($result==1) {
            ...
            $filedata= DB::table('attachment_activity_monitoring')
                    ...
                    $f=0;
              foreach($filedata as $file){
                $filepath = Config::get('app.site_root').'uploads/attachments'.'/'.$file->attachment_file;
                if(!file_exists($filepath)){
                 $f=1;
                  $filelist=$filelist.' '.$file->attachment_file.",";
              }
              }
               if($f == 1){
                return Redirect::back()->with('message', SiteHelpers::alert('error', ' file '.$filelist.'not synchronized to server.Please remove it and upload again'));
    ...
    public function add_pending_approval_admin($id, $id1)
    {
        ...
        if ($input->radioStatus == 2) {
           ...
           foreach ($input->comment as $value1) {
            ...
            return Redirect::to('dashboard')->with('message', SiteHelpers::alert('error', 'Please Login to Access this Page'));
      ...
      $checkClose = DB::table('request_progress_status')
        ...
        if ($input->radioStatus == 3) {
           ...
           // $message = 'Rejected';
            ...
            $data_1 = array('firstname'=>$contactName,'description'=>$message,'assigned_task_by'=>$assignedby,'task_assigned_to'=>$assignedto,'url'=>$url,'request_id'=>$request_id,
              'Close_status'=>$close_status,'comment'=>$comment,'Customer'=>$MailCustomer,'Parts'=>$Mailparts);
...
public function getSubType($r_id){
 ...
 public function view_search_result_download($id) {
            ...
            $query = DB::table('changerequests')
                ...
                ->leftJoin('tb_businessMaster', 'tb_businessMaster.id', '=', 'changerequests.business')
                ->leftJoin('tbl_chage_sub_type', 'changerequests.change_sub_type', '=', 'tbl_chage_sub_type.sub_type_id')
                ->select('changerequests.*', 'tb_stakeholder.*','changerequests.status as cmstatus','tb_projectMaster.*', 'changerequests.request_id as r_id','changerequests.plant_code as plant', 'tb_departments.d_name', 'subdepartments.sub_dep_name','tb_change_stage.stage_name','tb_change_stage.change_stage_id', 'user1.first_name as authority1', 'user1.last_name as authority2', 'user2.first_name as authority_1', 'user2.last_name as authority_2', 'tbl_change_type.change_type_name', 'plant_code.plant_code', 'horizontal_deployment.*', 'add_update_initial_sheet.selected as indexlevel', 'add_update_initial_sheet.currentTime as impdate','tbl_chage_sub_type.*','tbl_chage_sub_type.sub_type_name as type_name','tb_businessMaster.busi_code as busi')
                ->orderBy('changerequests.request_id', 'DESC')
                ->orderBy('horizontal_deployment.created', 'DESC')
                ->where('changerequests.request_id', '=', $id);
           ...
           // echo '<pre>'; print_r($users);exit;
            ...
            $data['userjobs'] = array(
                ...
                'prjMgrComment' =>$this->getprjMgrComment($user->r_id),
                    'projrct_mgr'=>$this->get_overallRisk_authority($user->change_stage_id,$user->plant,$user->stakeholder,$user->proj_code),
