<?php
class trackingSheetParamController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'TrackingSheet';
	static $per_page	= '10';
	...
	{ 
		...
		$this->layout->nest('content','trackingSheetParam.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		$this->layout->nest('content','trackingSheetParam.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
	 echo "search";exit;
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','trackingSheetParam.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$data = $this->validatePost('tracking_sheet_date_param');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'TrackingSheet/add/'.$id.'?md='.$md.$trackUri :  'TrackingSheet?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('TrackingSheet/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDestroy()
	{
		...
		return Redirect::to('TrackingSheet');
	}			
